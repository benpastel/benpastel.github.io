...
As an author, it's JUST SO NICE to put up a PR, see it approved the next day with thoughtful comments, and then use your professional judgement to triage those comments, merge, and move on with your job.  It's motivating; you want to put up the next PR faster.  And when you're grateful for receiving fast and thoughtful reviews, it feels right to reciprocate.

High trust has downsides: it's tempting to skim.  A gnarly diff while the reviewer is particularly busy would often get a cursory look and an apology.  And it's easy for documentation to slip, because there's so much shared understanding.
...
This continued until we made a new hire, and trust fell apart pretty quickly.  Reviewing one of his PRs, I hammered a critique that he felt was unrelated to his diff and patronizing; then he blocked one of my PRs on a documentation critique that I felt was unrelated and low-priority.  We avoided tagging each other as reviewers for a while.   Until eventually we came to our senses and talked through our feelings in a call.  And our working relationship somewhat recovered, but it never felt good.

This drove home that we hadn't really figured out code review.  We had reached effectiveness by circumstance of working together on a small, stable team for so long, and outside of that context I hadn't learned much.
...
As a reviewer you could combat this by reading the whole diff and trying to hold it all in your mind before your first comment.  But that's exhausting.  Besides, understandable code is the author's responsibility.  Your main goal should be to help them make it easier to understand.  Making code easier to understand is as effective at squashing bugs as straight up searching for bugs.

So, as a reviewer, it's better to embrace partial understanding, and use it to drive comments.  Look for a reading sequence that starts at some abstraction, like a new API endpoint, or new type, or function headers.  What's unclear about the behavior inside that abstraction?  Resist the urge to read ahead; instead, try to articulate your confusion at that snapshot in time.
...
What I wish I had done: read the endpoint documentation first and paused before examining the query.  "From this explanation I don't understand what order the results will be in, which matters for the limit+offset.  Can you explain in the docstring?"  Later I'd see the actual query, and add a follow up.  "How about an explicit ORDER BY here to match whatever the documented ordering is?"  Simple, low-risk fix; approve with comments.

In this approach, I'm 95% confident they would have fixed it without pushback.  I'm not saying they messed up; I'm saying I didn't understand, so could the code please clear up my confusion?  If they ignore that request, well, maybe there's some critical triage context they forgot to explain ("we need this up for a demo ASAP!").   Or maybe we disagree philosophically how much analytics accuracy matters and should talk it through face to face.  In any case, the risk is small and it's not worth damaging trust just in case.
